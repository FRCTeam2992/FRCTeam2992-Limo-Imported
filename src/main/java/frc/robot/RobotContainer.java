...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    // SmartDashboard.putData("Increase Second Shooter Speed", new
    // ChangeSecondaryShooterSpeed(mShooter, 50));
    // SmartDashboard.putData("Decrease Second Shooter Speed", new
    // ChangeSecondaryShooterSpeed(mShooter, -50));
     ...
     */
    // -Triggers
    JoystickButton autoAimButton = new JoystickButton(controller0, XboxController.Button.kRightBumper.value);
     ...
     * mShooter, cargoBallInterpolator));
     */
    JoystickButton autoShootButton = new JoystickButton(controller0,
        XboxController.Button.kRightBumper.value);
    autoShootButton.whileActiveContinuous(new AutoShoot(mCargoFunnel,
        mBottomLift,
        mShooter, mShooterHood, mTurret, mDrivetrain));

    // autoShootButton.whileActiveContinuous(new SetSwerveAngleSafe(mDrivetrain, 45,
    // -45, -45, 45));

    // JoystickButton panicIntakeButton1 = new JoystickButton(controller0,
    // XboxController.Button.kRightBumper.value);
    // panicIntakeButton1.whileHeld(new PanicIntake(mIntake, mIntakeDeploy,
    // mBottomLift, mCargoFunnel));

    JoystickButton slowModeButton = new JoystickButton(controller0, XboxController.Button.kLeftBumper.value);
    slowModeButton.whenPressed(new SetSlowMode(mDrivetrain, true));
    slowModeButton.whenReleased(new SetSlowMode(mDrivetrain, false));

    // JoystickButton intakeOrientCameraButton = new JoystickButton(controller0,
    ...
    JoystickButton increaseMainShooterSpeed = new JoystickButton(controller0,
        XboxController.Button.kY.value);
    increaseMainShooterSpeed.whenPressed(new ChangeMainShooterSpeed(mShooter,
        50));

    JoystickButton decreaseMainShooterSpeed = new JoystickButton(controller0,
        XboxController.Button.kB.value);
    decreaseMainShooterSpeed.whenPressed(new ChangeMainShooterSpeed(mShooter,
        -50));
    /*
    ...
    fieldOrientButton.onTrue(new ResetGyro(mDrivetrain));

    /*
     * controller1 Buttons
     */
    // -Triggers and Bumpers
    // Trigger dejamButton = new Trigger();

    JoystickButton dejamButton = new JoystickButton(controller1, XboxController.Button.kRightBumper.value);
    dejamButton.whileActiveContinuous(new DejamBallPath(mIntake, mCargoFunnel, mBottomLift, mIntakeDeploy));
    ...
    // true);
    // lowGoalShotButton.whileActiveOnce(new SetShooterSpeedTargets(mShooter, 1200,
    // 0), true);
    ...
    moveHoodUpButton.whileHeld(new MoveHood(mShooterHood, .25));

    POVButton moveHoodDownButton = new POVButton(controller1, 180);
    moveHoodDownButton.whileHeld(new MoveHood(mShooterHood, -.25));
    ...
    JoystickButton startShooterButton = new JoystickButton(controller1,
        XboxController.Button.kX.value);
    startShooterButton.whenPressed(new SetShooterCommanded(mShooter, true));

    JoystickButton autoIntakeButton = new JoystickButton(controller1, XboxController.Button.kA.value);
    autoIntakeButton.whenPressed(new AutoIntake(mIntake, mCargoFunnel, mBottomLift, mIntakeDeploy, true));
    // autoIntakeButton.whenPressed(new ChangeIntakeState(mIntakeDeploy, true),
    // true);

    JoystickButton stopAutoIntakeButton = new JoystickButton(controller1, XboxController.Button.kB.value);
    stopAutoIntakeButton.whenPressed(new StopAutoIntake(mIntake, mCargoFunnel, mBottomLift, mIntakeDeploy));

    JoystickButton stopShooterButton = new JoystickButton(controller1,
        XboxController.Button.kY.value);
    stopShooterButton.whenPressed(new SetShooterCommanded(mShooter, false));
    ...
    // JoystickButton reverseIntakeButton = new JoystickButton(controller1,
    // XboxController.Button.kRightStick.value);
    // reverseIntakeButton.whenPressed(new AutoIntake(mIntake, mCargoFunnel,
    // mBottomLift, mTopLift, mIntakeDeploy, false));

    /*
     * SmartDashBoard Buttons
     */
    SmartDashboard.putData("0 Wheels", new SetSwerveAngle(mDrivetrain, 0, 0, 0, 0));
    // SmartDashboard.putData("90 Wheels", new SetSwerveAngle(mDrivetrain, 90, 90,
    // 90, 90));
    // SmartDashboard.putData("180 Wheels", new SetSwerveAngle(mDrivetrain, 180,
    // 180, 180, 180));
    // SmartDashboard.putData("270 Wheels", new SetSwerveAngle(mDrivetrain, 270,
    // 270, 270, 270));

    // SmartDashboard.putData("Up 1 Angle", new NewHoodTarget(mShooterHood,
    // Math.floor(mShooterHood.getHoodTarget() + 1)));
    // SmartDashboard.putData("Down 1 Angle", new NewHoodTarget(mShooterHood,
    // Math.floor(mShooterHood.getHoodTarget() + -1)));

    // SmartDashboard.putData("Deploy Intake", new ChangeIntakeState(mIntakeDeploy,
    // true));
    // SmartDashboard.putData("Retract Intake", new ChangeIntakeState(mIntakeDeploy,
    // false));
    // SmartDashboard.putData("Reset Intake Encoder", new
    // ResetIntakeDeployEncoder(mIntakeDeploy));
    ...
    // Command testPathCommand =
    // mDrivetrain.followTrajectoryCommand(mDrivetrain.testPath, false);

    SmartDashboard.putData("Test Path Planner Path",
        new FollowTrajectoryCommand(mDrivetrain, mDrivetrain.testPath, false));
     ...
     * SmartDashboard.putData("Bottom Hood", new MoveHoodToAngle(mShooterHood,
     * -140.0));
  ...
  private void setupAutoSelector() {
    ...
    // Command driveStraightNoShootAuto = new AutoFollowPath(mDrivetrain, new
    // StraightPath(2.0, 0).generateSwerveTrajectory(), true, false, 0.0);
    // Command testPathAuto = new AutoFollowPath(mDrivetrain, new
    // TestPath(mDrivetrain, 90.0).generateSwerveTrajectory(), true, true, 90.0);
    // Command p3s1mAuto = new AutoP3S1M(mShooterHood, mShooter, mTurret,
    // cargoBallInterpolator, mCargoFunnel, mTopLift, mBottomLift,
    // mDrivetrain, mIntake, mIntakeDeploy);
    Command threeBallAuto = new ThreeBallAuto(mShooterHood, mShooter, mTurret,
        cargoBallInterpolator, mCargoFunnel,
        mBottomLift, mDrivetrain, mIntake, mIntakeDeploy);
    Command twoBallAuto = new TwoBallAuto(mShooterHood, mShooter, mTurret,
        cargoBallInterpolator, mCargoFunnel,
        mBottomLift, mDrivetrain, mIntake, mIntakeDeploy);
    Command fiveBallAuto = new FiveBallAuto(mShooterHood, mShooter, mTurret,
        cargoBallInterpolator, mCargoFunnel,
        mBottomLift, mDrivetrain, mIntake, mIntakeDeploy);
    autoChooser = new SendableChooser<>();

    autoChooser.setDefaultOption("Do Nothing", null);
    // autoChooser.addOption("Drive Straight (No Shoot)",
    // driveStraightNoShootAuto);
  ...
  public void initInterpolator() {
    ...
    // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(limeLightDistance,
    // Main, Backspin, Hood, realDistance));
  ...
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(37.7, 1750, 2650,
  // -152, 24.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(47.6, 1700, 2550,
  // -125, 48.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(71.6, 1900, 2800,
  // -78.5, 72.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(95.7, 1800, 3050,
  // 2, 96.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(119.4, 2200, 2850,
  // 110, 120.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(141.5, 2450, 3000,
  // 135.6, 144.0));//2350, 2850
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(165, 2600, 3250,
  // 148.5, 168.0));//2600, 3100
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(188.9, 2750, 3450,
  // 148.5, 192.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(213, 3050, 3500,
  // 148.5, 216.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(236, 3250, 3500,
  // 148.5, 240.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(265, 3300, 3850,
  // 148.5, 264.0));
  // cargoBallInterpolator.addDataPoint(new CargoBallDataPoint(280, 3450, 4700,
  // 138, 288.0));
